...
test('circuit device steps via mcp', async () => {
  ...
  assert.equal(q.circuit.steps.device, 'exact-amplitudes')
  ...
  assert.equal(q.circuit.register.kind, 'exact-amplitudes')
...
test('circuit ui via mcp', { timeout: 60_000 }, async () => {
  ...
  assert.equal(json.docs.documentation.includes('exact state-vector computation'), true)
  ...
  assert.equal(json.circuit.register.kind, 'exact-amplitudes')
