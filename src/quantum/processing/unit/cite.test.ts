...
test('paper and zenodo cite', () => {
  ...
  assert.equal(readme.includes('Up to 448 tools'), true)
...
test('paste in free AI chat', async (t) => {
  await t.test('AI sees JSON unit not HTML', async () => {
    ...
    assert.equal(page.docs.abstract.includes('Reads need no auth; storage writes need a Bearer token'), true)
    ...
    assert.equal(page.docs.documentation.includes('Possible only in quantum'), false)
