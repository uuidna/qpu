...
test('proof of concept — empty seat, fused env, VE reflections occupy the square', () => {
  ...
  assert.equal(P.concept.pwa, true)
  assert.equal(P.concept.installable, true)
  assert.equal(P.concept.display, 'standalone')
  assert.equal(P.concept.fractal, true)
  assert.equal(P.concept.replica, false)
...
test('proof of work — compare occupies BASE; a failing receipt refuses complete', () => {
  ...
  assert.equal(P.work.pwaCache, VE_FACES)
  assert.equal(P.work.pwaShortcuts, VE_FACES)
  assert.equal(P.work.pwaCompare, true)
